...
class _Property3State extends ConsumerState<Property3Page> {
  ...
  List<String> elemTitle = [
    "Bought without the intention \nof resale, but the first \nsteps towards this \nhave been taken",
    ...
    return ElevatedButton(
      onPressed: () {
        ref
            ...
            .setPurchasedNotForResaling(setValues(controllers[0].text));
        ref
            .read(zakatOnPropertyProvider.notifier)
            .setUsedAfterNisab(setValues(controllers[1].text));
        ref
            .read(zakatOnPropertyProvider.notifier)
            .setRentMoney(setValues(controllers[2].text));
        ref
            .read(zakatOnPropertyProvider.notifier)
            .setTaxesValue(setValues(controllers[3].text));
        performPostRequest();
      },
    ...
    return Column(
      children: [
        ...[
          ...
          for (int i = 0; i < 4; i++) enterField(controllers[i], elemTitle[i]),
