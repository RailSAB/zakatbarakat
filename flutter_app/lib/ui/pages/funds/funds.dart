...
class _FundsState extends ConsumerState<FundsPage> {
  ...
  Future getData() async {
    final response =
        await http.get(Uri.parse('http://10.90.137.169:8000/funds/get-funds'));
    try {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CustomAppBar(pageTitle: 'Recommended Funds', appBarHeight: 70),
      body: Padding(
  ...
  Widget button() {
    return ElevatedButton(
      ...
      style: ElevatedButton.styleFrom(
          minimumSize:
              const Size(double.infinity, 60)), // Adjusted for full width
